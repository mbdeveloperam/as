	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'ادد' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'از قبل توسط من محافظت میشد♥.', ok_cb, false)
			end
			print("سوپر گروه  "..msg.to.print_name.."("..msg.to.id..") ادد شد.")
		...
		if matches[1] == 'ریموو' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه ادد نشده که پاک کنید. /n ..msg.to.print_name..', ok_cb, false)
			end
			print("سوپر گروه "..msg.to.print_name.."("..msg.to.id..") پاک شد.")
			superrem(msg)
			rem_mutes(msg.to.id)
		end
		...
		if matches[1] == "درباره گروه" then
			...
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] در خواست کرد  درباره ی گگروه را ببیند.")
		...
		if matches[1] == "ادمین ها" then
			...
			member_type = 'Admins'
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] درخواست کرد تا اعضای ادمین هارا ببیند.")
		...
		if matches[1] == "مالک" then
			...
			if not group_owner then
				return "مالک وجود ندارد برای ثبت مالک به ادمین ب/ویید"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] used /owner")
			return "مالک گروه: ["..group_owner..']'
		end

		if matches[1] == "لیست مود" then
		...
		if matches[1] == "کی" and not matches[2] and is_momod(msg) then
			local user_id = msg.from.peer_id
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested SuperGroup users list")
			channel_get_users(receiver, callback_who, {receiver = receiver})
		end

		if matches[1] == "اخراجی ها" and is_momod(msg) then
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested Kicked users list")
			channel_get_kicked(receiver, callback_kicked, {receiver = receiver})
		end

		if matches[1] == 'دلیت' and is_momod(msg) then
		...
		if matches[1] == 'بلاک' or matches[1] == 'کیک' and is_momod(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
				...
				get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'بلاک' or matches[1] == 'کیک' and string.match(matches[2], '^%d+$') then
		...
		if matches[1] == 'اخراج من' then
			if msg.to.type == 'channel' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] درخواست اخراج کرد و اخراج شد")
		...
		if matches[1] == 'ست لینک' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return 'لینک جدید گروه رو ارسال کن'
		end
		...
		if matches[1] == "اینوایت" and is_sudo(msg) then
			local cbres_extra = {
		...
		if matches[1] == 'کیک' and is_momod(msg) then
			...
			elseif matches[1] == 'ست ادمین' and string.match(matches[2], '^%d+$') then
			--[[]	local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'ست ادمین' and not string.match(matches[2], '^%d+$') then
				--[[local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'setadmin'
				}
		...
		if matches[1] == 'بای ادمین' then
			...
			elseif matches[1] == 'بای ادمین' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'demoteadmin'
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'بای ادمین' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'demoteadmin'
				}
		...
		if matches[1] == 'ست مالک' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
			...
			elseif matches[1] == 'ست مالک' and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'ست مالک' and not string.match(matches[2], '^%d+$') then
				local	get_cmd = 'setowner'
				local	msg = msg
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'بای ترفیع' then
			if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
				return "فقظ ادمین یا مالک"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'demote',
			...
			elseif matches[1] == 'بای ترفیع' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
		...
		if matches[1] == "ست نیم" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "ست ابات" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "ست یوزر نیم" and is_admin1(msg) then
		...
		if matches[1] == 'ست قانون' and is_momod(msg) then
			rules = matches[2]
			local target = msg.to.id
		...
		if matches[1] == 'ست فتو' and is_momod(msg) then
		...
		if matches[1] == 'باز' and is_momod(msg) then
			local target = msg.to.id
			     if matches[2] == 'همه' then
      	local dsafemode ={
        unlock_group_links(msg, data, target),
		...
		if matches[1] == 'ساکت' and is_owner(msg) then
			local chat_id = msg.to.id
			...
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "ساکت کردن "..msg_type.."  فعال شد"
				else
					return "ساکت کردن "..msg_type.." فعال بود"
				end
			end
		end
		if matches[1] == 'ساکت باز' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'audio' then
			local msg_type = 'Audio'
			...
			local msg_type = 'All'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return "ساکت بودن "..msg_type.." غیر فعال شد"
				else
					return "ساکت بودن "..msg_type.." غیر فعال بود"
				end
			end
		end


		if matches[1] == "سایلنت" or matches[1] == "سایلنت باز" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "سایلنت" or matches[1] == "سایلنت باز" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] removed ["..user_id.."] from the muted users list")
					return "["..user_id.."] از لیست سایلنت درامد"
				elseif is_momod(msg) then
					mute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] added ["..user_id.."] to the muted users list")
					return "["..user_id.."] ادد شد به لیست سایلنت"
				end
			elseif matches[1] == "سایلنت" or matches[1] == "سایلنت باز" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == "موت لیست" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == "سایلنت لیست" and is_momod(msg) then
			local chat_id = msg.to.id
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested SuperGroup mutelist")
			return muted_user_list(chat_id)
		end

		if matches[1] == 'ستینگ' and is_momod(msg) then
			local target = msg.to.id
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested SuperGroup settings ")
			return show_supergroup_settingsmod(msg, target)
		end

		if matches[1] == 'قانون' then
